...
@pytest.mark.parametrize("output_model", [LLMOutput, LLMReasoningOutput])
async def test_run_llm_handles_empty_response(monkeypatch: pytest.MonkeyPatch, content: str | None, output_model: type[LLMOutput]) -> None:
    """Missing classifications should retain usage and report an execution error."""
    ...
    assert result.info == {"error_message": "LLM returned an empty classification response"}  # noqa: S101
