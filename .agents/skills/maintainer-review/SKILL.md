...
description: Assess an openai-guardrails-python GitHub issue or pull request as a maintainer. Use to verify the claimed need and practical impact, compare supported alternatives or competing approaches, separate code quality from repository readiness, recommend the maintainer action, and draft a copy-ready comment when evidence, changes, or closure should be requested.
---
...
- **Unsupported**: The outcome belongs outside the library contract or at a provider, adapter, or caller-owned layer.
...
Do not treat a test proving that new code can work as evidence that the feature is needed. A fake OpenAI client, manually constructed response or stream event, mock, or new regression test can establish code-path reachability and implementation correctness; it does not by itself establish realistic provider behavior, user reach, frequency, practical consequence, demand, or a material preventive outcome.
...
For Guardrails changes, inspect every affected public variant before recommending a merge: sync and async clients, streaming and non-streaming calls, Chat Completions and Responses, OpenAI and Azure construction, tripwire propagation and suppression, configuration validation, and Agents SDK integration. Review only the variants that share the changed boundary; do not assume one path proves the others.

Treat an explicit public `Literal`, enum, discriminated union, or equivalent static type restriction as evidence that other values are outside the supported contract. Avoid adding duplicate client-side runtime validation solely to reject values that the public type already excludes. Require evidence that the library itself ingests untyped data, that fail-fast behavior before side effects protects a documented contract or material invariant, or that the invalid value causes meaningful impact on a supported path. The host language's ability to bypass type hints, pass adversarial runtime objects, or mutate attributes after construction is not by itself sufficient reason to add permanent validation branches and tests.
...
Claims such as "this could sleep forever," "this could overflow," or "this might disable a limit" are insufficient without proving the realistic source of the value and the complete supported path to the consequence. Do not treat a security label as an exception by itself: identify the trust boundary, who can control the input, how it reaches the library, and the concrete protected outcome. A malformed value from an actually untrusted wire boundary may justify a fix when that trace is complete; a hypothetical hostile provider, monkeypatched object, or manually constructed payload does not by itself do so.
...
For repository-specific runtime invariants, start with the Guardrails runtime boundaries in `AGENTS.md`, then verify the current claim against the remote change, current code, tests, docs, and release boundary. Do not infer current issue or pull-request status from local notes or prior review artifacts.
