...
class SkillContractTest(unittest.TestCase):
    ...
    def test_full_verification_waits_for_clean_review(self) -> None:
        required_text = (
            ...
            "finding-bearing",
            "defer `make lint`, `uv run mypy src tests`, `uv run pyright`, `make tests`, "
            "repository-wide builds, examples runners, and integration suites until step 19 "
            "establishes clean review",
            "Do not run `make tests` or repository-wide mypy or pyright during an iterative "
            "review round",
    ...
    def test_iterative_review_uses_focused_checks_only(self) -> None:
        required_text = (
            ...
            "Do not run repository-wide lint, type checking, builds, integration suites, or "
            "`make tests`",
    ...
    def test_round_budget_preserves_history_across_feedback_cycles(self) -> None:
        required_text = (
            ...
            "A same-round retry is valid only when that value and the authorized budget history match",
    ...
    def test_semantic_clean_credit_fails_closed_on_dependency_changes(self) -> None:
        required_text = (
            ...
            "`public-client-contract`, `guardrail-execution`, `streaming-lifecycle`, "
            "`security-sensitive-data`, `evals-mcp`, `tests-examples`, and "
            "`release-metadata`",
