...
finish_step() {
  ...
  if [ "${status}" -eq 0 ]; then
    clear_step "${idx}"
    echo "${name} passed in $((now - start_time))s."
    return 0
  fi

  echo "code-change-verification: ${name} failed with exit code ${status} after $((now - start_time))s." >&2
...
check_for_missing_reporters() {
  ...
  for idx in "${!STEP_PIDS[@]}"; do
    ...
    echo "code-change-verification: ${name} exited before reporting completion status after $((now - start_time))s." >&2
    echo "--- ${name} log (last 80 lines) ---" >&2
    tail -n 80 "${log_file}" >&2 || true
    stop_running_steps
    return "${step_status}"
...
echo "Running lint, mypy, pyright, and tests in parallel..."
start_step "make lint" make lint
start_step "uv run mypy src tests" uv run mypy src tests
start_step "uv run pyright" uv run pyright
start_step "make tests" make tests
set +e
wait_for_parallel_steps
EXIT_STATUS=$?
set -e
