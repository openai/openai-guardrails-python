...
class GuardrailMetricsCalculator(MetricsCalculator):
    ...
    def _calculate_guardrail_metrics(self, results: Sequence[SampleResult], name: str) -> GuardrailMetrics:
        ...
        true_positives = sum(
            1 for r in results if r.expected_triggers.get(name) and r.triggered.get(name)
        )
        false_positives = sum(
            1 for r in results if not r.expected_triggers.get(name) and r.triggered.get(name)
        )
        false_negatives = sum(
            1 for r in results if r.expected_triggers.get(name) and not r.triggered.get(name)
        )
        true_negatives = sum(
            1 for r in results
            if not r.expected_triggers.get(name) and not r.triggered.get(name)
        )
